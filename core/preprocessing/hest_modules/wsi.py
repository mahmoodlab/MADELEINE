## Code taken from https://github.com/mahmoodlab/HEST/tree/main wsi.py
## TODO replace by an independent package
...
class WSIPatcher:
    ...
    def _compute_masked(self, coords) -> None:
        ...
        top_left = coords
        top_right = coords + np.array([self.patch_size_src, 0])
        bottom_left = coords + np.array([0, self.patch_size_src])
        bottom_right = coords + np.array([self.patch_size_src, self.patch_size_src])
        
        # Combine all corner coordinates
        corners = np.stack([top_left, top_right, bottom_left, bottom_right], axis=1).reshape(-1, 2)
        ...
        valid_mask = gpd.GeoSeries(points).within(union_mask).values
        valid_mask = valid_mask.reshape(-1, 4).any(axis=1)  # Check any corner within mask
        valid_patches_nb = valid_mask.sum()
        valid_coords = coords[valid_mask]
...
def get_pixel_size(slide):
    ...
    try:
        ...
        pixel_size = float(slide.properties.get(openslide.PROPERTY_NAME_MPP_X, 0))
